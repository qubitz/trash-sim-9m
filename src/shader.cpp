...
{
  std::ifstream shaderLoader;
  std::string   s, temp;

  if (ShaderType == GL_VERTEX_SHADER)
  {
    shaderLoader.open("shaders/Fragment_lighting_vertex_shader");
  }
  else if (ShaderType == GL_FRAGMENT_SHADER)
  {
    shaderLoader.open("shaders/Fragment_lighting_fragment_shader");
  }

  while (std::getline(shaderLoader, temp))
  {
    s += (temp + '\n');
  }
